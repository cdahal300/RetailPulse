...
- Commands: `POST /api/v1/tenants/{tenantId}/stores/{storeId}/analytics/reprocess` requires owner authorization, `X-RetailPulse-Command-Id`, scoped sale-event payload, and applies a deterministic fact correction. Durable replay job storage and operator audit are still pending.
