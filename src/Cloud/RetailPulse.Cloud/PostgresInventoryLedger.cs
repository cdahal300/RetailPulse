...
public sealed class PostgresInventoryLedger : IInventoryLedgerRepository
{
    ...
    private static async Task EnsureSchemaAsync(NpgsqlConnection connection, CancellationToken cancellationToken)
    {
        ...
        command.CommandText = "CREATE TABLE IF NOT EXISTS inventory_balances (tenant_id TEXT NOT NULL, store_id TEXT NOT NULL, product_id TEXT NOT NULL, quantity INTEGER NOT NULL, version INTEGER NOT NULL, PRIMARY KEY (tenant_id, store_id, product_id)); CREATE TABLE IF NOT EXISTS inventory_movements (tenant_id TEXT NOT NULL, store_id TEXT NOT NULL, movement_id TEXT NOT NULL, product_id TEXT NOT NULL, quantity_delta INTEGER NOT NULL, reason TEXT NOT NULL, effective_at TIMESTAMPTZ NOT NULL, expected_version INTEGER NOT NULL, aggregate_version INTEGER NOT NULL, actor_id TEXT NOT NULL, role TEXT NOT NULL, command_id TEXT NOT NULL, correlation_id TEXT NOT NULL, PRIMARY KEY (tenant_id, store_id, movement_id), UNIQUE (tenant_id, store_id, command_id)); CREATE TABLE IF NOT EXISTS inventory_thresholds (tenant_id TEXT NOT NULL, store_id TEXT NOT NULL, product_id TEXT NOT NULL, minimum_quantity INTEGER NOT NULL, version INTEGER NOT NULL, PRIMARY KEY (tenant_id, store_id, product_id));";
        await command.ExecuteNonQueryAsync(cancellationToken);
    }
